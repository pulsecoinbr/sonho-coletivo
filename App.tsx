...
const AppWithProvider: React.FC = () => {
  const [campaigns, setCampaigns] = useState<Campaign[]>(() => {
    try {
      ...
      if (saved) {
        ...
        // More robust validation to prevent crashes from malformed legacy data
        if (
    ...
    // If parsing fails or validation fails, clear the bad data and start fresh
  ...
  const [currentUser, setCurrentUser] = useState<User | null>(() => {
    try {
      ...
      // Validate essential user data, including the address object
      if (
          parsedUser && 
          parsedUser.id && 
          parsedUser.name && 
      ...
      // If data is malformed, clear it and log out
...
Vou continuar a correção do arquivo App.tsx para garantir que todas as rotas funcionem corretamente:

<dyad-write path="App.tsx" description="Continuing to fix the App.tsx file with all routes">
import React, { Component, useState, useCallback, useEffect } from 'react';
import { HashRouter, Routes, Route, useNavigate } from 'react-router-dom';
import Header from './components/Header';
import Footer from './components/Footer';
...
const AppWithProvider: React.FC = () => {
  ...
  return (
    <AppContext.Provider value={{ campaigns, testimonials, addCampaign, addDonation, users, currentUser, login, logout, register }}>
